...
class _MobilePlayerScreenState extends State<MobilePlayerScreen>
  ...
  Timer? _hideTimer;
  /// Periodically save stream URL + position (links expire; app kill skips dispose).
  ...
  /// Stremio live TV (`/stream/tv/...`): long HLS buffer stalls otherwise spin forever.
  ...
  Future<void> _configureMpvProperties() async {
    ...
    if (_isStremioLiveTv) {
      // Live IPTV: prefer lowest variant — fewer stalls on congested / CDN paths
      // (aggregators like FlixNest often return multi-bitrate HLS).
